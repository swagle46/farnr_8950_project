---
title: "FANR 8950 Final Project: Georgia FIA Tree Data Analysis"
...
spp_name <-  read_csv("REF_SPECIES.csv")
...
We filter the observations with STATUSCD 1 and 2 to focus on live and standing trees only. Some variables were added to facilitate analysis:  
...
A custom function was used to summarize minimum, maximum, mean, and standard deviation. Then we summarize the DBH and height per plot for 295 plots and presented in a nice readable table.  
...
Then, we visualized the diameter and height relationship for all species with different colors in scatterplot with showing size class (small, medium and large) detail .    
...
ggplot(ga_tree_named, aes(x = dbh_cm, y = height_m, color = species)) +
  ...
  labs(title = "Height-diameter relationship of various species in GA",
    x = "DBH (cm)",
    y = "Height (m)"
  ) +
...
ggplot(biomass_top_spp, aes(x = dbh_cm, y = height_m, color = scientific_name, size = biomass_kg)) +
  ...
  labs(title = "Tree height-diameter relationship of top three pine species with biomass in GA", x = "DBH (cm)", y = "Tree height(m)", color = "Species", size = "Biomass (kg)") +
  theme_minimal()
...
We plotted the available coordinates in the plot dataset to visualize the locations of the sample plots of FIA in Georgia. There were many latitude and longitude data in one plot. So, at first, we select the one latitude and longitude coordinate per plot. Then, we plotted the latitude and longitude of the plots. This spatial visualization is useful for assessing geographic coverage of the plots. 
...
- Georgia FIA tree data has 189 species and 295 plots.  
...
- Temporal and spatial analysis and visualization provide insights for operational planning and sampling efficiency.  
